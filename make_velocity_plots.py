...
with MyProcessPool(12) as p:
    p.map(lambda i: run_axionCAMB("CMAB_run4plots_{}".format(i), params[i], "CMAB_run4plots_log_{}".format(i)), range(0, len(params)))

with MyProcessPool(12) as p:
    outputs = p.map(lambda i:  run_mpv("MPV_run4plots_log_{}".format(i), "MPV_run4plots_{}".format(i), "CMAB_run4plots_{}".format(i), params[i], window, minClusterMass[stage], z_vals), range(0, len(params)))
